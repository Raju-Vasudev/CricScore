...
const OverItem = ({ index, runsInOver, wicketsInOver, over }) => {
  return (
    <Card sx={{ marginBottom: '8px', backgroundColor: '#f5f5f5', padding: '10x' }}>
      <CardContent>
        <ListItem key={index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', marginLeft:'0px', paddingLeft: '0px'}}>
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '6px' , flexWrap:'wrap', marginLeft:'0px'}}>
          <Chip
              label={`Over ${index + 1}`}
              sx={{
